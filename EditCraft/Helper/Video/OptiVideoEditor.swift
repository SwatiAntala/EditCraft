...
class OptiVideoEditor: NSObject {
    ...
    func addfiltertoVideo(strfiltername : String, strUrl : URL, success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        var outputURL = documentDirectory.appendingPathComponent("EffectVideo_\(uniqueDirectoryName)")
        do {
...
//        print("video length: \(length) seconds")
        ...
        let documentDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        let uniqueDirectoryName = UUID().uuidString
        var outputURL = documentDirectory.appendingPathComponent("TrimVideo_\(uniqueDirectoryName)")
        do {
            try FileManager.default.createDirectory(at: outputURL, withIntermediateDirectories: true, attributes: nil)
            outputURL = outputURL.appendingPathComponent("\(outputURL.lastPathComponent).m4v")
        }catch let error {
...
//        print("video length: \(length) seconds")
        ...
        if compatiblePresets.contains(AVAssetExportPresetMediumQuality) {
            ...
            let uniqueDirectoryName = UUID().uuidString
            var outputURL = documentDirectory.appendingPathComponent("TrimAudio_\(uniqueDirectoryName)")
    ...
    func videoScaleAssetSpeed(fromURL url: URL,  by scale: Float64, success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        do {
            ...
            let uniqueDirectoryName = UUID().uuidString
            var outputURL = documentDirectory.appendingPathComponent("SpeedVideo_\(uniqueDirectoryName)")
    ...
    func mergeVideoWithAudio(videoUrl: URL, audioUrl: URL, success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        let uniqueDirectoryName = UUID().uuidString
        var outputURL = documentDirectory.appendingPathComponent("MergeVideowithAudio_\(uniqueDirectoryName)")
    ...
    func mergeTwoVideosArry(arrayVideos:[AVAsset], success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        let uniqueDirectoryName = UUID().uuidString
        var outputURL = documentDirectory.appendingPathComponent("MergeTwoVideos_\(uniqueDirectoryName)")
    ...
    func transitionAnimation(videoUrl: URL, animation:Bool, type: Int, playerSize: CGRect,success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        let uniqueDirectoryName = UUID().uuidString
        var outputURL = documentDirectory.appendingPathComponent("TransitionVideo_\(uniqueDirectoryName)")
    ...
    func addStickerorTexttoVideo(videoUrl: URL, watermarkText text : String, imageName name : String, position : Int,  success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        let uniqueDirectoryName = UUID().uuidString
        var outputURL = documentDirectory.appendingPathComponent("StickerVideo_\(uniqueDirectoryName)")
...
//        }
    ...
    // MARK: - Save video inside Photos library to a specific album and retrieve the localIdentifier
    func save(videoUrl: URL, toAlbum titled: String, completionHandler: @escaping (Bool, String, Error?) -> ()) {
        // Retrieve the album or create it if it doesn't exist
        getAlbum(title: titled, completionHandler: { (album) in
            DispatchQueue.global(qos: .background).async {
                var localIdentifier: String = ""
                
                PHPhotoLibrary.shared().performChanges({
                    // Create a request to save the video
                    let assetRequest = PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: videoUrl)
                    
                    // Capture the localIdentifier of the created asset
                    if let placeholder = assetRequest?.placeholderForCreatedAsset {
                        localIdentifier = placeholder.localIdentifier
                    }
                    
                    // Add the asset to the specified album
                    if let assets = assetRequest?.placeholderForCreatedAsset.map({ [$0] as NSArray }),
                       let album = album,
                       let albumChangeRequest = PHAssetCollectionChangeRequest(for: album) {
                        albumChangeRequest.addAssets(assets)
                    }
                }) { (success, error) in
                    // Call the completion handler with success status, localIdentifier, and error
                    completionHandler(success, localIdentifier, error)
                }
            }
        })
    }
